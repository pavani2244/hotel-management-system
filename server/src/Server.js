...
class Server {
  ...
  async mountRoutes() {
    ...
    this.api.use('/api/dish', DishRoute);
    this.api.use('/api/categories', CategoriesRoute);

    // Swagger Setup. Will run only in development mode
  ...
  async startServer() {
    ...
    this.httpServer.listen(this.options.port, async () => {
      logger.info('APPLICATION_STARTED', {
        meta: {
          ENV: this.options.nodeEnv.toUpperCase(),
